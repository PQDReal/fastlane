    ...
    <div className="relative text-sm font-medium text-gray-700" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setOpen(false) }}>
      ...
      <div className="relative mt-2">
        ...
        <input
          ...
          className="w-full rounded-xl border border-gray-200 bg-white py-3 pl-10 pr-10 font-normal focus:border-[#836100] focus:outline-none disabled:cursor-not-allowed disabled:bg-gray-50"
        />
      ...
      {open && !disabled && !loading && (
        <div id={listId} role="listbox" className="absolute z-30 mt-2 max-h-60 w-full overflow-y-auto rounded-xl border border-gray-200 bg-white p-1 shadow-xl">
          {filtered.length ? filtered.map((option) => {
            ...
            return <button key={option.code} type="button" role="option" aria-selected={active} onClick={() => { onChange(option); setQuery(option.name); setOpen(false) }} className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-sm font-normal transition active:scale-[0.99] ${active ? 'bg-[#836100]/10 text-[#836100]' : 'text-gray-700 hover:bg-gray-50'}`}><span>{option.name}</span>{active && <Check className="h-4 w-4" />}</button>
