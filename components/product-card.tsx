...
export function ProductCard({ name, desc, price, image, hot, href = '#' }: { name: string; desc: string; price: string; image: string; hot?: boolean; href?: string }) {
  return (
    <article className="group h-full flex flex-col items-center text-center cursor-pointer pb-8">
      <Link href={href} className="relative aspect-[4/3] w-full flex items-center justify-center overflow-hidden rounded-2xl bg-gray-50/50">
        ...
        <img 
          src={image} 
          alt={name} 
          className="relative z-10 w-full h-full object-cover transition-transform duration-700 ease-[0.16,1,0.3,1] group-hover:scale-[1.05]" 
        />
      </Link>
