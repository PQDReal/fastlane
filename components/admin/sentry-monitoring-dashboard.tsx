...
import { Activity, Gauge, Monitor, RefreshCw, Server, TriangleAlert } from 'lucide-react'
...
function LatencyPanel({ title, summary, icon: Icon }: { title: string; summary: LatencySummary; icon: typeof Monitor }) {
  ...
  const metrics = [['Trung bình', summary.avgMs], ['p50', summary.p50Ms], ['p95', summary.p95Ms], ['p99', summary.p99Ms]] as const
  return <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between gap-2"><div className="flex items-center gap-2"><Icon size={18} className="text-[#b88a08]"/><h2 className="font-semibold text-slate-900">{title}</h2></div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${status.style}`}>{status.label}</span></div><div className="grid grid-cols-2 divide-x divide-slate-200 rounded-lg bg-slate-50 py-4 text-center sm:grid-cols-4">{metrics.map(([label, value]) => <div key={label}><p className="text-xs font-semibold uppercase text-slate-500">{label}</p><p className="mt-1 text-lg font-bold text-slate-900">{latency(value)}</p></div>)}</div><div className="mt-4 flex justify-between text-sm text-slate-500"><span>{count(summary.requestCount)} request</span><span>Lỗi: <strong className="text-slate-700">{percentage(summary.failureRate)}</strong></span></div></section>
}
function SlowTable({ title, rows }: { title: string; rows: SlowTransaction[] }) {
  return <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 px-5 py-4"><h2 className="font-semibold text-slate-900">{title}</h2></div>{rows.length === 0 ? <p className="px-5 py-8 text-center text-sm text-slate-500">Chưa có request phù hợp trong khoảng thời gian này.</p> : <div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-5 py-3">Method / route</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Requests</th><th className="px-3 py-3">Trung bình</th><th className="px-3 py-3">p50</th><th className="px-3 py-3">p95</th><th className="px-3 py-3">p99</th><th className="px-3 py-3">Tỷ lệ lỗi</th><th className="px-3 py-3">Đánh giá</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((row, index) => { const status = latencyStatus(row.p95Ms); return <tr key={`${row.name}-${row.method}-${index}`} className="hover:bg-slate-50"><td className="max-w-xs px-5 py-3"><div className="flex items-center gap-2"><span className="rounded bg-slate-100 px-1.5 py-0.5 text-xs font-bold text-slate-700">{row.method || '—'}</span><p className="truncate font-medium text-slate-800">{row.name}</p></div><p className="mt-1 text-xs text-slate-400">{row.operation}</p></td><td className="px-3 py-3 text-slate-600">{row.statusCode ?? '—'}</td><td className="px-3 py-3 text-slate-600">{count(row.requestCount)}</td><td className="px-3 py-3 text-slate-600">{latency(row.avgMs)}</td><td className="px-3 py-3 text-slate-600">{latency(row.p50Ms)}</td><td className="px-3 py-3 font-semibold text-slate-900">{latency(row.p95Ms)}</td><td className="px-3 py-3 text-slate-600">{latency(row.p99Ms)}</td><td className="px-3 py-3 text-slate-600">{percentage(row.failureRate)}</td><td className="px-3 py-3"><span className={`whitespace-nowrap rounded-full px-2 py-1 text-xs font-semibold ${status.style}`}>{status.label}</span></td></tr>})}</tbody></table></div>}</section>
}
