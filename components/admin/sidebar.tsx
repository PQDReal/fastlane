...
export function AdminSidebar({ open = false, collapsed = false, onClose, user }: Props) {
  ...
  return (
    <aside
      className={`group/sidebar fixed inset-y-0 left-0 z-50 flex w-64 flex-col overflow-hidden border-r border-slate-800 bg-slate-900 text-slate-300 transition-[width,transform] duration-300 ease-out md:translate-x-0 ${collapsed ? 'md:w-20' : 'md:w-64'} ${open ? 'translate-x-0 shadow-2xl' : '-translate-x-full'}`}
    >
      <div className={`relative flex h-16 shrink-0 items-center border-b border-slate-800 bg-slate-950 px-6`}>
        <Link href="/admin" className="group flex min-w-0 items-center gap-2.5" aria-label="FASTLANE Admin">
          ...
          />
          <span
            className={`font-display mt-0.5 overflow-hidden whitespace-nowrap text-xl font-bold tracking-[0.06em] text-[#b88a08] transition-[width,opacity] duration-200 ${collapsed ? 'md:w-0 md:opacity-0' : 'w-[130px] opacity-100'}`}
          >
            FASTLANE
          </span>
        </Link>
        <button
          type="button"
          onClick={onClose}
          className="ml-auto rounded-md p-2 text-slate-400 hover:bg-slate-800 hover:text-white md:hidden"
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex h-10 items-center rounded-md text-sm font-medium transition-colors gap-3 px-3 ${active ? 'bg-brand-600/10 text-brand-400' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
              <item.icon size={19} className={`shrink-0 ${active ? 'text-brand-500' : 'text-slate-500'}`} />
              <span
                className={`overflow-hidden whitespace-nowrap transition-[width,opacity] duration-200 ${collapsed ? 'md:w-0 md:opacity-0' : 'w-auto opacity-100'}`}
              >
                {item.name}
              </span>
            </Link>
      ...
      <div className="space-y-1 border-t border-slate-800 p-3">
        <a
          ...
          className={`flex h-10 w-full items-center rounded-md text-sm font-medium text-slate-400 transition-colors hover:bg-slate-800 hover:text-red-400 gap-3 px-3`}
        >
          <LogOut size={19} className="shrink-0 text-slate-500" />
          <span
            className={`overflow-hidden whitespace-nowrap transition-[width,opacity] duration-200 ${collapsed ? 'md:w-0 md:opacity-0' : ''}`}
          >
            Đăng xuất
          </span>
        </a>
      ...
      <div
        className={`flex min-h-16 items-center border-t border-slate-900 bg-slate-950 p-3 gap-3`}
      ...
      >
        ...
        <div
          className={`min-w-0 flex-1 overflow-hidden transition-[width,opacity] duration-200 ${collapsed ? 'md:w-0 md:flex-none md:opacity-0' : ''}`}
        >
