  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex items-start justify-between gap-3">
      <div className="flex items-center gap-2">
        ...
        <div><h2 className="font-semibold text-slate-900">Redis cache</h2><p className="text-sm text-slate-500">So sánh hiệu năng cache với đường tải dữ liệu nguồn</p></div>
      </div>
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <Metric label="Có Redis (HIT)" value={formatMs(total?.avgCacheReadMs ?? null)}/>
      <Metric label="Không cache" value={formatMs(total?.avgSourceLoadMs ?? null)}/>
    </div>

    {total && <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <div className="flex items-center gap-3 rounded-lg border border-emerald-200 bg-emerald-50 p-4"><Zap className="text-emerald-600" size={21}/><div><p className="text-xs font-medium text-emerald-700">Mức cải thiện ước tính</p><p className="text-xl font-bold text-emerald-900">{formatPercent(total.improvementPercent)}</p></div></div>
