...
const SELECT='id,auth0_subject,email,full_name,phone_number,role,status,created_at,updated_at'
const PUBLIC='id,email,full_name,phone_number,role,status,created_at,updated_at'
...
export async function PATCH(request:Request,context:Context){
 ...
 const updates:Record<string,string|null>={updated_at:new Date().toISOString()},auth0Updates:{email?:string;fullName?:string;phoneNumber?:string|null;blocked?:boolean;role?:'ADMIN'|'CUSTOMER'}={}
 if(input.email!==undefined){const value=typeof input.email==='string'?input.email.trim().toLowerCase():'';if(!EMAIL.test(value))return NextResponse.json({error:'Email không hợp lệ.'},{status:400});updates.email=value;auth0Updates.email=value}
