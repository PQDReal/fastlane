...
export async function PATCH(request: Request, context: Context) {
  ...
  try {
    ...
    const hasFormFields = ['code', 'name', 'description', 'type', 'value', 'applicableProductType', 'maxDiscountAmount', 'minimumOrderAmount', 'startsAt', 'endsAt'].some((key) => key in body)
    if (hasFormFields) {
      ...
      if (!applicableProductType) return NextResponse.json({ error: 'Loại sản phẩm áp dụng không hợp lệ.' }, { status: 400 })
      ...
      Object.assign(updates, { code, name, description: description || null, type, value, applicable_product_type: applicableProductType, max_discount_amount: maxDiscountAmount, minimum_order_amount: minimumOrderAmount, starts_at: startsAt.toISOString(), ends_at: endsAt.toISOString() })
    }
