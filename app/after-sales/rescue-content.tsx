...
const CONTACT_METHODS = [
  { title: 'Gọi tổng đài cứu hộ', text: 'Liên hệ trực tiếp để cung cấp vị trí và tình trạng xe.', icon: PhoneCall },
  { title: 'Ứng dụng hoặc màn hình xe', text: 'Chọn mục Cứu hộ trên kênh được hỗ trợ của dòng xe.', icon: Smartphone },
  { title: 'Nút SOS', text: 'Dùng trong trường hợp khẩn cấp trên các mẫu xe có trang bị.', icon: ShieldAlert },
]
...
const EMERGENCY_GUIDES = ['VF 3', 'VF 5', 'VF 6', 'VF 7', 'VF 8', 'VF 9', 'VF e34', 'Limo Green và VF Limo']
...
export function RescueContent({ items, onOpenManuals, onOpenWorkshops }: RescueContentProps) {
  const policy = items.find((item) => item.vehicleType === 'car' || item.vehicleType === 'all') ?? items[0]
  const hotline = policy?.hotline || '1900 23 23 89'
  ...
  return (
    <article className="space-y-14 animate-in fade-in duration-200 sm:space-y-16">
      <header className="border-b border-slate-200 pb-8">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#836100]">Ô tô · Hỗ trợ khẩn cấp</p>
        <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">Thông tin cứu hộ</h2>
        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
          Thông tin liên hệ, phạm vi hỗ trợ, eCall và hướng dẫn ứng phó dành cho xe ô tô VinFast.
        </p>
      </header>

      <section id="rescue-information" className="scroll-mt-28 border-b border-slate-200 pb-14 sm:pb-16">
        <Heading
          index="01"
          title="Thông tin cứu hộ 24/7"
          description={
            policy?.description || 'Dịch vụ cứu hộ dành cho xe ô tô tại các thị trường có triển khai dịch vụ.'
          }
        />
        ...
        <div className="grid gap-5 bg-slate-900 p-7 text-white sm:p-9 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.15em] text-red-300">
              <span className="h-2 w-2 animate-pulse rounded-full bg-red-400" /> Hotline cứu hộ{' '}
              {policy?.operatingHours || '24/7'}
            </div>
            <a
              href={`tel:${phoneHref}`}
              className="mt-3 block text-4xl font-semibold text-[#e6b32e] transition hover:text-white sm:text-5xl"
            >
              {hotline}
            </a>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
              Dịch vụ áp dụng cho chủ sở hữu hoặc người được ủy quyền sử dụng xe; phạm vi cụ thể được tổng đài xác nhận
              theo từng tình huống.
            </p>
          </div>
          <a
            href={`tel:${phoneHref}`}
            className="inline-flex items-center justify-center gap-2 bg-[#836100] px-6 py-4 text-sm font-bold uppercase tracking-wider text-white transition hover:bg-[#6c4f00] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e6b32e] active:scale-[0.98]"
          >
        ...
        <div className="mt-6 grid gap-px overflow-hidden border border-slate-200 bg-slate-200 md:grid-cols-2">
          <div className="bg-white p-6">
            ...
            <h4 className="mt-4 font-semibold text-slate-900">Phạm vi hỗ trợ</h4>
            <ul className="mt-4 space-y-3">
              ...
              {policy?.mobileChargingSupport && (
                <li className="flex gap-3 text-sm leading-6 text-slate-700">
                  <BatteryCharging size={16} className="mt-1 shrink-0 text-[#836100]" />
                  <span>Có hỗ trợ giải pháp năng lượng lưu động theo điều kiện vận hành.</span>
                </li>
              )}
            </ul>
          </div>
          <div className="bg-white p-6">
            ...
            <h4 className="mt-4 font-semibold text-slate-900">Điều kiện cần lưu ý</h4>
            <ul className="mt-4 space-y-3">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {CONTACT_METHODS.map((method) => {
            ...
            return (
              <div key={method.title} className="border border-slate-200 bg-white p-5">
                <Icon size={20} className="text-[#836100]" />
                <h4 className="mt-4 text-sm font-semibold text-slate-900">{method.title}</h4>
                <p className="mt-2 text-xs leading-5 text-slate-600">{method.text}</p>
              </div>
            )
          })}
        </div>
      ...
      <section id="rescue-ecall" className="scroll-mt-28 border-b border-slate-200 pb-14 sm:pb-16">
        <Heading
          index="02"
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {ECALL_TIMELINE.map((step, index) => (
            <div key={step.title} className="relative border border-slate-200 bg-white p-5">
              <span className="text-xs font-bold uppercase tracking-wider text-[#836100]">{step.time}</span>
      ...
      <section id="rescue-guide" className="scroll-mt-28 border-b border-slate-200 pb-14 sm:pb-16">
        <Heading
          index="03"
        ...
        <div className="border border-slate-200 bg-white p-6 sm:p-8">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="font-semibold text-slate-900">Danh mục tài liệu theo dòng xe</h4>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Các tài liệu này nằm trong nhóm hướng dẫn sử dụng, đúng với phạm vi tài liệu dự án đang giữ lại.
              </p>
            </div>
          </div>
      ...
      <section id="rescue-support" className="scroll-mt-28">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          >
            ...
            <p className="mt-4 text-xs font-bold uppercase tracking-wider text-slate-500">Chuẩn bị trước tình huống</p>
            <p className="mt-2 flex items-center gap-2 font-semibold text-slate-900 group-hover:text-[#836100]">
