...
const CAR_PROCESS = [
  {
    number: '01',
    title: 'Đặt hẹn sửa chữa',
    text: 'Chọn thời gian và cung cấp trước tình trạng xe để xưởng chủ động tiếp nhận.',
  },
  {
    number: '02',
  ...
  { number: '04', title: 'Bàn giao xe', text: 'Kiểm tra chất lượng, giải thích hạng mục và bàn giao hồ sơ dịch vụ.' },
  {
    number: '05',
...
const CAR_REPAIR_TYPES = [
  {
    ...
    description: 'Chẩn đoán và sửa chữa hệ thống điện, cơ khí, truyền động cùng các hạng mục vận hành của xe.',
    icon: Wrench,
    details: [
  ...
  },
  {
    ...
    description: 'Khắc phục hư hỏng thân vỏ, khung gầm và bề mặt sơn bằng quy trình kiểm soát theo từng mức độ.',
    icon: Paintbrush,
    details: [
  ...
  },
  {
    ...
    description: 'Kiểm tra khi xe lệch lái, rung vô lăng, lốp mòn không đều hoặc sau va chạm và sửa chữa hệ thống gầm.',
    icon: CircleGauge,
    details: ['Đo góc đặt bánh xe', 'Cân bằng động', 'Khuyến nghị kiểm tra định kỳ'],
  },
]
...
export function RepairContent({ vehicleType, items, onBook, onOpenManuals, onOpenWorkshops }: RepairContentProps) {
  ...
  return (
    <article className="space-y-14 animate-in fade-in duration-200 sm:space-y-16">
      <header className="border-b border-slate-200 pb-8">
        ...
        <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">Dịch vụ sửa chữa</h2>
        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
          Dịch vụ chính hãng kết hợp kỹ thuật viên chuyên môn, thiết bị phù hợp và phụ tùng có khả năng truy xuất.
        </p>
        <button
          type="button"
          onClick={() => onBook('Sửa chữa')}
        ...
        >
          <Calendar size={15} /> Đặt lịch sửa chữa
        </button>
      </header>

      {isCar && (
        <section id="repair-process" className="scroll-mt-28 border-b border-slate-200 pb-14 sm:pb-16">
          <Heading
            index="01"
            title="Quy trình dịch vụ sửa chữa"
            description="Quy trình 5 bước từ đặt hẹn đến chăm sóc sau sửa chữa."
          />
      ...
      <section id="repair-types" className="scroll-mt-28 border-b border-slate-200 pb-14 sm:pb-16">
        <Heading
          index={isCar ? '02' : '01'}
          title={isCar ? 'Phân loại dịch vụ sửa chữa' : 'Dịch vụ sửa chữa VinFast'}
          description={
            isCar
        ...
        {isCar ? (
                ...
                <div key={service.title} className="bg-white p-6">
                  ...
                  <ul className="mt-5 space-y-2 border-t border-slate-100 pt-4">
                    {service.details.map((detail) => (
                      <li key={detail} className="flex gap-2 text-xs leading-5 text-slate-700">
                        <CheckCircle2 size={14} className="mt-0.5 shrink-0 text-[#836100]" />
                        <span>{detail}</span>
                      </li>
                    ))}
                  </ul>
          ...
          <div className="grid gap-5 border border-slate-200 bg-white p-6 md:grid-cols-[1fr_0.9fr] md:p-8">
            <div>
              ...
              <h4 className="mt-4 text-xl font-semibold text-slate-900">Sửa chữa chung xe máy điện</h4>
              <p className="mt-3 text-sm leading-7 text-slate-600">
                Xưởng tiếp nhận, chẩn đoán và sửa chữa các hệ thống vận hành bằng quy trình dành riêng cho xe máy điện.
              </p>
            </div>
        ...
        {items.length > 0 && (
          <div className="mt-6 border border-slate-200 bg-slate-50 p-5 sm:p-6">
            <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Dữ liệu dịch vụ đã publish</p>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              {items.map((item) => (
                <div key={item.id} className="border border-slate-200 bg-white p-5">
                  <span className="text-xs font-semibold text-[#836100]">{item.badge}</span>
                  <h4 className="mt-2 font-semibold text-slate-900">{item.title}</h4>
                  <p className="mt-2 text-sm leading-6 text-slate-600">{item.description}</p>
                  {item.features.length > 0 && (
                    <p className="mt-3 text-xs leading-5 text-slate-500">{item.features.join(' • ')}</p>
                  )}
                </div>
              ))}
            </div>
      ...
      <section id="repair-commitment" className="scroll-mt-28 border-b border-slate-200 pb-14 sm:pb-16">
        <Heading
          index={isCar ? '03' : '02'}
        ...
        <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="bg-slate-900 p-7 text-white">
            ...
            <p className="mt-3 text-xs leading-5 text-slate-300">
              Mức tham chiếu đã gồm VAT; quyền lợi thực tế phụ thuộc điều kiện áp dụng và xác nhận của xưởng.
            </p>
          </div>
              ...
              {isCar ? (
                <>
                  <li className="flex gap-3">
                    <ShieldCheck size={17} className="mt-1 shrink-0 text-[#836100]" />
                    <span>Sửa chữa động cơ hoặc PDU/POD có phương án hỗ trợ di chuyển theo chính sách.</span>
                  </li>
                  <li className="flex gap-3">
                    <ShieldCheck size={17} className="mt-1 shrink-0 text-[#836100]" />
                    <span>
                      Sửa chữa pin ưu tiên pin mượn; phương án thay thế được áp dụng khi không có pin phù hợp.
                    </span>
                  </li>
                  <li className="flex gap-3">
                    <ShieldCheck size={17} className="mt-1 shrink-0 text-[#836100]" />
                    <span>Điều kiện cho xe kinh doanh vận tải và xe mượn được xác nhận riêng khi tiếp nhận.</span>
                  </li>
                </>
              ) : (
                <>
                  <li className="flex gap-3">
                    <ShieldCheck size={17} className="mt-1 shrink-0 text-[#836100]" />
                    <span>Khi sửa chữa pin, khách hàng có thể được hỗ trợ pin mượn nếu xưởng có sẵn.</span>
                  </li>
                  <li className="flex gap-3">
                    <ShieldCheck size={17} className="mt-1 shrink-0 text-[#836100]" />
                    <span>Trường hợp hỗ trợ pin mượn không đồng thời áp dụng hỗ trợ tài chính.</span>
                  </li>
                </>
              )}
              <li className="flex gap-3">
                <ShieldCheck size={17} className="mt-1 shrink-0 text-[#836100]" />
                <span>Không tính các ngày nghỉ lễ, Tết theo quy định hiện hành.</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      {isCar && (
        <section id="repair-parts" className="scroll-mt-28 border-b border-slate-200 pb-14 sm:pb-16">
          ...
          <div className="grid gap-px overflow-hidden border border-slate-200 bg-slate-200 sm:grid-cols-3">
            <div className="bg-white p-6">
              ...
              <h4 className="mt-4 font-semibold text-slate-900">Kiểm soát chất lượng</h4>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Linh kiện được đánh giá theo quy trình phê duyệt sản phẩm phù hợp với ngành ô tô.
              </p>
            </div>
            <div className="bg-white p-6">
              <Settings size={21} className="text-[#836100]" />
              <h4 className="mt-4 font-semibold text-slate-900">Tiêu chuẩn tương thích</h4>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Thông số, độ bền và yêu cầu an toàn được kiểm soát trước khi đưa vào dịch vụ.
              </p>
            </div>
            <div className="bg-white p-6">
              <CheckCircle2 size={21} className="text-[#836100]" />
              <h4 className="mt-4 font-semibold text-slate-900">Đối tác ngành ô tô</h4>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Ưu tiên hệ thống nhà cung cấp đáp ứng các chứng nhận quản lý chất lượng và môi trường.
              </p>
            </div>
          </div>
        </section>
