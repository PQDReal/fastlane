...
function cancellationAudit(row: Row): OrderCancellationAudit | null {
...
export default async function AdminAccessoryOrdersPage() {
  ...
  const legacyResult = auditSchemaUnavailable
    ? await supabase.from('orders').select(baseSelection).order('created_at', { ascending: false })
    : null
  const result = legacyResult ?? auditedResult

  const orders: AdminAccessoryOrder[] = ((result.data ?? []) as unknown as Row[]).map((row) => {
