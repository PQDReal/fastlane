          ...
          <div className="space-y-8">
            ...
            <div className="rounded-xl border border-brand-100 bg-brand-50/30 p-5">
              <div className="flex items-center justify-between gap-4">
                <div><h4 className="text-sm font-bold text-slate-900">Tùy chỉnh bộ thông số mặc định</h4><p className="mt-1 text-xs text-slate-500">Bỏ chọn để ẩn chỉ số hoặc sửa nhãn cho sản phẩm này.</p></div>
                <Button type="button" size="sm" variant="outline" onClick={addSpecField}><Plus size={14} className="mr-1" /> Thêm chỉ số</Button>
              </div>
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {form.specification_fields.map((field) => <div key={field.key} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-2">
