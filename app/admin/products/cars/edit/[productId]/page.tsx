...
interface VersionEntry {
  ...
  // Legacy input retained while editing saved drafts; API ignores it.
...
export default function EditCarPage({ params }: { params: Promise<{ productId: string }> }) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/v1/admin/cars/${productId}`, { cache: 'no-store' })
        if (!res.ok) {
          const errData = await res.json()
          throw new Error(errData.error || 'Không thể tải dữ liệu xe ô tô.')
        }
  ...
  const validateForm = () => {
    ...
    if (form.versions.some((version) => selectedColorsFor(version).some((color) => selectedInteriorsFor(version, color).length === 0))) {
      return 'Mỗi cặp phiên bản–ngoại thất phải có ít nhất một màu nội thất hợp lệ (Tab 4)'
    }
    return null
  }
  ...
  const handleSaveProduct = async () => {
    ...
    setIsSaving(true)
    try {
      const response = await fetch(`/api/v1/admin/cars/${productId}`, {
        method: 'PATCH',
  ...
  if (isLoading) {
    return (
      <div className="flex h-[400px] items-center justify-center">
        ...
        <span className="ml-2 text-sm text-slate-600">Đang tải thông tin xe...</span>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6 pb-24">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-950">Chỉnh sửa xe ô tô điện</h1>
            <p className="text-xs text-slate-500 mt-1">Chỉnh sửa thông số, màu sắc, phiên bản và hình ảnh của xe {form.name || ''}.</p>
          </div>
        </div>
