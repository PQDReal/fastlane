...
type InventorySummary = { totalRows: number; totalQuantity: number; statusCounts: Partial<Record<InventoryStatus, number>> }
type InventoryProductOption = { id: string; name: string; productType: string }
type InventoryValueOption = { value: string; label: string }
type InventoryFilterOptions = {
  typeCounts: Record<AdminInventoryProductType, number>
  products: InventoryProductOption[]
  variants: InventoryValueOption[]
  colors: InventoryValueOption[]
  interiorColors: InventoryValueOption[]
}
...
function statusLabel(status: InventoryStatus) { return inventoryStatusLabel(status) }
...
export default function AdminInventoryPage() {
  ...
  const [summary, setSummary] = useState<InventorySummary>({ totalRows: 0, totalQuantity: 0, statusCounts: {} })
  ...
  useEffect(() => {
    ...
    async function loadInventory() {
      ...
      try {
        ...
        if (interiorFilter !== 'ALL') params.set('interiorColor', interiorFilter)
        if (statusFilter !== 'ALL') params.set('status', statusFilter)
        if (activityFilter !== 'ALL') params.set('activity', activityFilter)
        if (currentCursor) params.set('cursor', currentCursor)
        const response = await fetch(`/api/v1/admin/inventory/query?${params.toString()}`, { cache: 'no-store', signal: controller.signal })
        if (!response.ok) throw new Error(await responseError(response))
        const body: { data: InventoryQueryResponse } = await response.json()
        if (active) {
        ...
        if (active && !(cause instanceof DOMException && cause.name === 'AbortError')) notify('error', TEXT.loadError, cause instanceof Error ? cause.message : TEXT.loadError)
      } finally { if (active) setIsLoading(false) }
    }
    void loadInventory()
    return () => { active = false; controller.abort() }
  }, [activityFilter, colorFilter, currentCursor, debouncedSearch, interiorFilter, notify, pageSize, productFilter, statusFilter, typeFilter, variantFilter])
  ...
  useEffect(() => {
    ...
    async function loadFilterOptions() {
      try {
        ...
        if (typeFilter !== 'ALL') params.set('productType', typeFilter)
        if (productFilter !== 'ALL') params.set('productId', productFilter)
        const response = await fetch(`/api/v1/admin/inventory/filter-options?${params.toString()}`, {
          cache: 'no-store',
          signal: controller.signal,
        })
        if (!response.ok) throw new Error(await responseError(response))
        const body: { data: InventoryFilterOptions } = await response.json()
        if (active) setFilterOptions(body.data)
      } catch (cause) {
        if (active && !(cause instanceof DOMException && cause.name === 'AbortError')) {
          notify('error', 'Không thể tải bộ lọc tồn kho', cause instanceof Error ? cause.message : TEXT.loadError)
        }
      }
    }
  ...
  }, [notify, productFilter, typeFilter])

  const typeCounts = filterOptions.typeCounts
  const productOptions = useMemo(() => filterOptions.products.map((product) => product.name), [filterOptions.products])
  const variantOptions = useMemo(() => filterOptions.variants.map((variant) => variant.value), [filterOptions.variants])
  const colorOptions = useMemo(() => filterOptions.colors.map((color) => color.value), [filterOptions.colors])
  const interiorOptions = useMemo(() => filterOptions.interiorColors.map((color) => color.value), [filterOptions.interiorColors])
  ...
  function selectProduct(nextProduct: string) {
    const selected = filterOptions.products.find((product) => product.name === nextProduct)
    setProductFilter(selected?.id ?? 'ALL')
    setProductFilterLabel(selected?.name ?? '')
    setVariantFilter('ALL')
    setColorFilter('ALL')
    setInteriorFilter('ALL')
    resetPagination()
  ...
  function selectStatus(nextStatus: 'ALL' | InventoryStatus) {
    setStatusFilter(nextStatus)
    resetPagination()
  }

  function selectActivity(nextActivity: AdminInventoryActivity) {
    setActivityFilter(nextActivity)
    resetPagination()
  }

  function selectSearch(nextSearch: string) {
  ...
  function selectInterior(nextInterior: string) {
    setInteriorFilter(nextInterior)
    resetPagination()
  }
  ...
  function isVehicleInventoryItem(item: InventoryItem) {
    return ['CAR', 'BIKE'].includes(canonicalInventoryProductType(item.productType)) && Boolean(item.productId)
  }
  ...
  function openAdjustment(item: InventoryItem) {
    if (isVehicleInventoryItem(item)) {
      const editorPath = canonicalInventoryProductType(item.productType) === 'BIKE'
  ...
  async function saveAdjustment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setItems((current) => current.map((item) => item.variantId === updated.variantId ? { ...item, onHandQuantity: updated.onHandQuantity, updatedAt: updated.updatedAt, variantIsActive: updated.variantIsActive, isActive: updated.variantIsActive && item.productIsActive, inventoryStatus: !updated.variantIsActive || !item.productIsActive ? 'INACTIVE' : updated.onHandQuantity <= 0 ? 'OUT_OF_STOCK' : updated.onHandQuantity <= 5 ? 'LOW_STOCK' : 'IN_STOCK' } : item))
  ...
  const cards = [{ label: TEXT.totalSku, value: summary.totalRows, Icon: Layers3, color: 'bg-blue-50 text-blue-600', filter: null }, { label: TEXT.totalQuantity, value: summary.totalQuantity, Icon: Boxes, color: 'bg-emerald-50 text-emerald-600', filter: null }, { label: TEXT.lowStock, value: summary.statusCounts.LOW_STOCK ?? 0, Icon: AlertTriangle, color: 'bg-amber-50 text-amber-600', filter: 'LOW_STOCK' as const }, { label: TEXT.outOfStock, value: summary.statusCounts.OUT_OF_STOCK ?? 0, Icon: ArchiveX, color: 'bg-red-50 text-red-600', filter: 'OUT_OF_STOCK' as const }, { label: TEXT.inactive, value: summary.statusCounts.INACTIVE ?? 0, Icon: Ban, color: 'bg-slate-100 text-slate-600', filter: 'INACTIVE' as const }]
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">{cards.map(({ label, value, Icon, color, filter }) => {
        ...
        const applyFilter = () => {
          if (filter === 'INACTIVE') { selectActivity(active ? 'ALL' : 'INACTIVE'); setStatusFilter('ALL'); return }
          selectActivity('ALL')
          selectStatus(active ? 'ALL' : filter)
        }
      ...
      <div className="min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="space-y-4 border-b border-slate-200 bg-slate-50/50 p-4">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
            <div className="relative min-w-0 flex-1"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="search" placeholder={TEXT.search} value={searchTerm} onChange={(event) => selectSearch(event.target.value)} className="h-10 w-full rounded-md border border-slate-200 bg-white pl-9 pr-4 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:flex xl:shrink-0">
              <label className="flex h-10 min-w-0 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-700"><Filter size={16} className="shrink-0 text-slate-400" /><span className="sr-only">Trạng thái tồn kho</span><select value={statusFilter} onChange={(event) => selectStatus(event.target.value as 'ALL' | InventoryStatus)} className="min-w-0 flex-1 bg-transparent focus:outline-none"><option value="ALL">{TEXT.allStatuses}</option>{(['IN_STOCK', 'LOW_STOCK', 'OUT_OF_STOCK', 'MISSING_INVENTORY', 'UNLINKED', 'INACTIVE'] as InventoryStatus[]).map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></label>
              <label className="flex h-10 min-w-0 items-center rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-700"><span className="sr-only">Trạng thái kinh doanh</span><select value={activityFilter} onChange={(event) => selectActivity(event.target.value as AdminInventoryActivity)} className="min-w-0 flex-1 bg-transparent focus:outline-none"><option value="ALL">{TEXT.allActivities}</option><option value="ACTIVE">Đang kinh doanh</option><option value="INACTIVE">Ngừng kinh doanh</option></select></label>
            </div>
          ...
          <div className={`grid gap-3 sm:grid-cols-2 ${showInteriorFilter ? 'xl:grid-cols-4' : showVehicleFilters ? 'xl:grid-cols-3' : 'xl:grid-cols-2'}`}>
            <div className="min-w-0 text-xs font-semibold text-slate-600"><span className="mb-1.5 block">Sản phẩm</span><InventoryProductCombobox options={productOptions} value={productFilter === 'ALL' ? 'ALL' : productFilterLabel} onChange={selectProduct} allLabel={TEXT.allProducts} ariaLabel="Tìm và chọn sản phẩm" /></div>
            ...
            {showVehicleFilters && <label className="min-w-0 text-xs font-semibold text-slate-600"><span className="mb-1.5 block">Màu ngoại thất</span><select value={colorFilter} onChange={(event) => selectColor(event.target.value)} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"><option value="ALL">{TEXT.allColors}</option>{colorOptions.map((color) => <option key={color} value={color}>{color}</option>)}</select></label>}
            {showInteriorFilter && <label className="min-w-0 text-xs font-semibold text-slate-600"><span className="mb-1.5 block">Màu nội thất</span><select value={interiorFilter} onChange={(event) => selectInterior(event.target.value)} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"><option value="ALL">{TEXT.allInteriors}</option>{interiorOptions.map((color) => <option key={color} value={color}>{color}</option>)}</select></label>}
          </div>

          <p className="text-xs text-slate-500">Đang hiển thị <span className="font-bold text-slate-700">{items.length}</span> trên {summary.totalRows} SKU. Các bộ lọc được kết hợp đồng thời.</p>
        </div>
        ...
        <table className="w-full table-fixed text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!isLoading && items.length === 0 && <tr><td colSpan={8} className="px-6 py-14 text-center text-slate-500">{TEXT.noData}</td></tr>}
          </tbody>
        </table>
        {!isLoading && items.length > 0 && (
          <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/50 px-4 py-3 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2">
              ...
              <select value={pageSize} onChange={(event) => selectPageSize(Number(event.target.value))} className="h-8 rounded-md border border-slate-200 bg-white px-2 focus:outline-none">
                <option value={10}>10</option><option value={20}>20</option><option value={50}>50</option>
              </select>
              <span className="hidden md:inline">· {TEXT.showing} {(safePage - 1) * pageSize + 1}–{(safePage - 1) * pageSize + items.length} / {summary.totalRows} {TEXT.results}</span>
            </div>
            <div className="flex items-center justify-between gap-3 sm:justify-end">
              <span>{TEXT.page} {safePage} {TEXT.of} {totalPages}</span>
              <div className="flex gap-1">
                <button type="button" onClick={goToPreviousPage} disabled={safePage === 1} className="flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Previous page"><ChevronLeft size={16} /></button>
                <button type="button" onClick={goToNextPage} disabled={!hasMore} className="flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Next page"><ChevronRight size={16} /></button>
              </div>
            </div>
          </div>
        )}
