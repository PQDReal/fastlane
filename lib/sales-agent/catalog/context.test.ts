...
describe('sales agent catalog context', () => {
  ...
  it('includes dataAsOf and typed product identity', () => {
    const result = serializeCatalogContext([{ id: 'p1', name: 'VF 8', slug: 'vf-8', productType: 'CAR', price: 900000000, availableQuantity: 2, facts: { 'powertrain.range': '471 km' }, dataAsOf: '2026-08-12T00:00:00.000Z' }])
  ...
  it('does not turn generic Vietnamese question words into a product filter', async () => {
    ...
    query.select.mockReturnValue(query); query.eq.mockReturnValue(query); query.limit.mockResolvedValue({ data: [{ id: 'p1', name: 'VF 8', slug: 'vf-8', product_type: 'CAR', displayed_price: 1, specifications: { powertrain: { range: '471 km' } }, product_variants: [] }], error: null })
