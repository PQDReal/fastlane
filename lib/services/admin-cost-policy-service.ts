...
export async function listCostPolicies(){const{data,error}=await getSupabaseAdmin().from('on_road_fee_policies').select('id,fee_type,name,vehicle_type,province_code,calculation_type,value,effective_from,effective_to,is_active,version,created_at,updated_at').order('effective_from',{ascending:false});return{fees:(data??[])as OnRoadFeePolicy[],error:error?'Chưa thể tải chính sách chi phí.':null}}
