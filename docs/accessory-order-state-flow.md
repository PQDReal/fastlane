# State machine đơn hàng phụ kiện

Tài liệu này mô tả luồng trạng thái mới của **đơn hàng phụ kiện**. Hệ thống lưu riêng ba nhóm trạng thái:
...
## 1. Luồng chính của đơn hàng

```mermaid
stateDiagram-v2
    [*] --> PENDING: Khách tạo đơn

    PENDING --> PAID: VNPay xác nhận thanh toán
    PENDING --> CANCELLED: Khách hoặc admin hủy

    PAID --> CONFIRMED: Admin xác nhận đơn
    PAID --> CANCELLED: Khách hoặc admin hủy

    CONFIRMED --> READY: Admin chọn "Giao hàng"
    CONFIRMED --> CANCELLED: Admin hoặc khách hủy

    READY --> DELIVERED: Admin chọn "Đã giao hàng"
...
|---|---|---|---|
| `PENDING` | Chưa thanh toán | Đơn đã tạo nhưng VNPay chưa xác nhận thanh toán | Thanh toán, thanh toán lại, hủy |
| `PAID` | Đã thanh toán | VNPay đã xác nhận giao dịch | Admin xác nhận hoặc hủy |
| `CONFIRMED` | Đã xác nhận, chờ lấy hàng | Admin đã tiếp nhận đơn | Bắt đầu giao hàng hoặc hủy |
| `READY` | Đang giao hàng | Đơn đang được giao cho khách | Đánh dấu đã giao hàng |
| `DELIVERED` | Hoàn thành | Đơn đã giao thành công | Không còn thao tác |
| `CANCELLED` | Đã hủy / Đang chờ hoàn tiền | Đơn đã dừng xử lý | Có thể hoàn tiền nếu đơn đã thanh toán |

Luồng mới tách việc giao hàng thành hai thao tác độc lập:

1. `ship`: chuyển `CONFIRMED → READY`.
2. `complete`: chuyển `READY → DELIVERED`.
...
Return URL phục vụ điều hướng giao diện; IPN server-to-server là nguồn cập nhật giao dịch độc lập với trình duyệt khách hàng.
...
Khi bật, hệ thống chỉ mô phỏng hoàn tất nếu API URL thực sự thuộc `sandbox.vnpayment.vn`, chữ ký hợp lệ, response code là `00`, loại giao dịch là refund (`02`/`03`) và trạng thái là `05` hoặc `06`. Không sử dụng cấu hình này với VNPay production.
...
| `CONFIRMED` | Admin chọn “Giao hàng” (`ship`) | `READY` | `NONE` |
| `CONFIRMED` | Admin hủy | `CANCELLED` | `PENDING` |
| `READY` | Admin chọn “Đã giao hàng” (`complete`) | `DELIVERED` | `NONE` |
