...
const inputPath = path.resolve(
  requestedInput ||
    (fs.existsSync(normalizedV6Path)
      ? normalizedV6Path
...
function rejectFact(name, predicate) {
  ...
  if (matching.length)
    failures.push(
      `${name}: found ${matching.length} forbidden fact(s): ${matching.map((fact) => fact.factId).join(', ')}`,
    )
}

expectFact(
  'hyphenated accumulator duration',
  (fact) =>
...
expectFact(
  'hyphenated accumulator distance',
  (fact) =>
    fact.subject === 'battery_12v' &&
...
rejectFact(
  'accumulator must not become high-voltage battery',
  (fact) =>
...
expectFact(
  'thousands separator preserves model sentence',
  (fact) =>
    fact.model === 'Fadil' &&
...
expectFact(
  'general maintenance does not inherit nearby component',
  (fact) =>
...
rejectFact(
  'general maintenance must not emit a nearby component duplicate',
  (fact) =>
    fact.sourceId === 'vinfast-maintenance-car' &&
    fact.subject !== 'vehicle' &&
    fact.action === 'scheduled_service' &&
    ((fact.valueNumeric === 12000 && fact.unit === 'km') || (fact.valueNumeric === 12 && fact.unit === 'month')),
)

rejectFact(
  'motorbike replacement-battery rows must not inherit replacement-part subject',
  (fact) =>
...
expectFact(
  'lexical yearly interval is normalized',
  (fact) =>
    fact.model === 'VF e34' &&
    fact.subject === 'battery_coolant' &&
    fact.action === 'inspect' &&
...
expectFact(
  'same-action alternatives share an interval group',
  (fact) =>
    fact.model === 'VF e34' &&
    fact.subject === 'battery_coolant' &&
    fact.action === 'inspect' &&
    ...
    facts.some(
      (other) =>
        other !== fact &&
        other.model === fact.model &&
        other.subject === fact.subject &&
        other.action === fact.action &&
        other.intervalGroupId === fact.intervalGroupId &&
        other.valueNumeric !== fact.valueNumeric,
    ),
)
expectFact(
  'temporal interval keeps group policy separate',
  (fact) =>
    fact.model === 'VF e34' &&
    fact.subject === 'battery_coolant' &&
    fact.action === 'inspect' &&
...
rejectFact(
  'temporal interval must not inherit distance policy from counterpart',
  (fact) =>
    fact.model === 'VF e34' &&
    fact.subject === 'battery_coolant' &&
    fact.action === 'inspect' &&
...
expectFact(
  'engine-oil yearly interval preserves dashboard warning alternative',
  (fact) =>
...
expectFact(
  'tire monthly interval preserves both event alternatives',
  (fact) =>
    fact.subject === 'tire' &&
    fact.action === 'inspect' &&
    fact.valueNumeric === 1 &&
    fact.unit === 'month' &&
    ['after_refueling', 'before_driving'].every((code) =>
      fact.alternativeTriggers?.some((trigger) => trigger.code === code),
    ),
)

// New semantic invariants from expert review
expectFact(
  'VF 8 and VF 8 The All New co-exist without dropping plain VF 8',
  (fact) => fact.model === 'VF 8' && fact.subject === 'vehicle' && fact.valueNumeric === 10 && fact.unit === 'year',
)
expectFact(
  'VF 8 The All New exists as separate entity',
  (fact) =>
    fact.model === 'VF 8 The All New' && fact.subject === 'vehicle' && fact.valueNumeric === 7 && fact.unit === 'year',
)

expectFact(
  'VF 3 vehicle standard warranty is 7 years',
  (fact) =>
    fact.model === 'VF 3' &&
    fact.subject === 'vehicle' &&
...
expectFact(
  'VF 3 high-voltage battery standard warranty is 8 years',
  (fact) =>
    fact.model === 'VF 3' &&
    fact.subject === 'battery' &&
    fact.usageCondition === 'standard_use' &&
...
rejectFact(
  'VF 3 battery duration must not leak into vehicle policy',
  (fact) =>
    fact.model === 'VF 3' &&
    fact.subject === 'vehicle' &&
    fact.usageCondition === 'standard_use' &&
...
expectFact(
  'VF EC Van vehicle warranty is 5 years',
  (fact) =>
...
expectFact(
  'VF EC Van vehicle warranty is limited to 130000 km',
  (fact) =>
    fact.model === 'VF EC Van' &&
    fact.subject === 'vehicle' &&
    fact.usageCondition === 'standard_use' &&
...
expectFact(
  'VF EC Van high-voltage battery warranty is 7 years',
  (fact) =>
    fact.model === 'VF EC Van' &&
    fact.subject === 'battery' &&
    fact.usageCondition === 'standard_use' &&
...
expectFact(
  'VF EC Van high-voltage battery warranty is limited to 160000 km',
  (fact) =>
    fact.model === 'VF EC Van' &&
    fact.subject === 'battery' &&
    fact.usageCondition === 'standard_use' &&
...
rejectFact(
  'VF EC Van battery terms must not leak into vehicle policy',
  (fact) =>
    fact.model === 'VF EC Van' &&
    fact.subject === 'vehicle' &&
    fact.usageCondition === 'standard_use' &&
...
expectFact(
  'customer-paid replacement parts retain their own 2-year scope',
  (fact) =>
    fact.subject === 'replacement_part' &&
...
expectFact(
  'post-delivery high-voltage battery retains its 4-year scope',
  (fact) =>
    fact.subject === 'battery' &&
...
expectFact(
  'VF 3 accessory unlimited distance',
  (fact) => fact.model === 'VF 3' && fact.subject === 'accessory' && fact.distancePolicy === 'unlimited',
)
expectFact(
  'Minio Green 12V battery unlimited distance',
  (fact) => fact.model === 'VF Minio Green' && fact.subject === 'battery_12v' && fact.distancePolicy === 'unlimited',
)

expectFact(
  'VF 7 emergency safety wait time model specific',
  (fact) => fact.model === 'VF 7' && fact.factType === 'emergency_safety_wait_time' && fact.valueNumeric === 5,
)
...
rejectFact(
  'percentage must use controlled semantic type',
  (fact) =>
    fact.unit === 'percent' &&
...
expectFact(
  'post-repair battery capacity floor keeps its own semantic type',
  (fact) =>
    fact.factType === 'battery_post_repair_capacity_floor' &&
    fact.action === 'post_repair_capacity_floor' &&
    fact.valueNumeric === 70 &&
    fact.unit === 'percent',
)
rejectFact(
  'post-repair battery guarantee must not become warranty eligibility threshold',
  (fact) =>
    fact.factType === 'battery_capacity_threshold' &&
    (fact.provenances || []).some((provenance) =>
      /(?:sửa chữa|thay thế)[\s\S]{0,240}phạm vi\s+bảo hành[\s\S]{0,240}(?:đảm bảo\s+)?dung lượng\s+pin/iu.test(
        provenance.excerpt || '',
      ),
    ),
)

const pdfProvenances = facts
  .flatMap((fact) => fact.provenances || [])
  .filter((provenance) => provenance.extractionMethod === 'pdf_text_layer')
assertionCount += 1
