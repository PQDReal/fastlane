...
if (data.normalizerVersion !== 'after-sales-facts-v5') warnings.push(`unexpected normalizerVersion ${data.normalizerVersion}`)
...
for (const fact of data.facts || []) {
  ...
  if (fact.intervalGroupId && !fact.intervalRelation) errors.push(`${factId}: intervalGroupId is missing intervalRelation`)
  if (fact.intervalGroupDistancePolicy !== null && !allowedDistancePolicies.has(fact.intervalGroupDistancePolicy)) errors.push(`${factId}: invalid intervalGroupDistancePolicy ${fact.intervalGroupDistancePolicy}`)
  if (fact.intervalGroupId && !fact.intervalGroupDistancePolicy) errors.push(`${factId}: intervalGroupId is missing intervalGroupDistancePolicy`)
  ...
  if (!allowedUsageConditions.has(fact.usageCondition)) errors.push(`${factId}: invalid usageCondition ${fact.usageCondition}`)
  if (!allowedApplicabilities.has(fact.applicability)) errors.push(`${factId}: invalid applicability ${fact.applicability}`)
  if (fact.policyEntity !== fact.subject) errors.push(`${factId}: policyEntity compatibility alias differs from subject`)
  if (!Number.isFinite(fact.valueNumeric) || fact.valueNumeric <= 0) errors.push(`${factId}: invalid numeric value`)
  if (!Number.isFinite(fact.confidence) || fact.confidence < 0 || fact.confidence > 1) errors.push(`${factId}: invalid confidence`)
  ...
  if (['battery_capacity_threshold', 'coverage_percentage'].includes(fact.factType) && fact.unit !== 'percent') {
  ...
  if (fact.unit === 'VND' && fact.factType !== 'service_price') errors.push(`${factId}: currency used by non-price fact`)
  if (fact.serviceType !== 'warranty' && fact.usageCondition !== 'general') errors.push(`${factId}: non-warranty fact has warranty usage condition`)
  if (fact.serviceType !== 'warranty' && fact.applicability !== 'general') errors.push(`${factId}: non-warranty fact has warranty applicability`)
  ...
  for (const [prefix, expectedSubject] of Object.entries(warrantySubjects)) {
    if (fact.factType.startsWith(prefix) && fact.subject !== expectedSubject) errors.push(`${factId}: ${fact.factType} requires subject ${expectedSubject}`)
  }
  if (fact.factType === 'battery_capacity_threshold' && fact.subject !== 'battery') errors.push(`${factId}: battery threshold requires battery subject`)

  if (fact.unit === 'year' && fact.valueNumeric > 30) warnings.push(`${factId}: unusually large year value ${fact.valueNumeric}`)
  if (fact.unit === 'month' && fact.valueNumeric > 240) warnings.push(`${factId}: unusually large month value ${fact.valueNumeric}`)
  if (fact.unit === 'day' && fact.valueNumeric > 3650) warnings.push(`${factId}: unusually large day value ${fact.valueNumeric}`)
  if (fact.unit === 'minute' && fact.valueNumeric > 10080) warnings.push(`${factId}: unusually large minute value ${fact.valueNumeric}`)
  if (fact.unit === 'km' && fact.valueNumeric > 2_000_000) warnings.push(`${factId}: unusually large distance ${fact.valueNumeric}`)
  if (fact.unit === 'VND' && fact.valueNumeric > 100_000_000_000) warnings.push(`${factId}: unusually large price ${fact.valueNumeric}`)

  if (!manifestSources.has(fact.sourceId)) errors.push(`${factId}: sourceId not found in manifest`)
  if (!Array.isArray(fact.sourceIds) || fact.sourceIds.length === 0) errors.push(`${factId}: sourceIds must be non-empty`)
  ...
  if (JSON.stringify(fact.provenance) !== JSON.stringify(fact.provenances[0])) errors.push(`${factId}: legacy provenance is not the first provenance`)
  ...
  for (const provenance of fact.provenances) {
    ...
    if (['asset_text_extraction', 'asset_ocr'].includes(provenance.origin)) {
      ...
      const matchingAsset = (source.assets || []).find(asset =>
