...
for (const source of manifest.sources) {
  ...
  if (fs.existsSync(file)) {
    ...
    if (snapshot.captureMethod === 'browserless_playwright' && snapshot.contentValidation?.status !== 'passed') {
      errors.push('Browserless capture is missing expected service-workshop content validation')
    }
    ...
    results.push({ sourceId: source.id, captureMethod: snapshot.captureMethod, providerFailures: (snapshot.acquisitionFailures || []).length, extractionScope: snapshot.extractionScope, textLength: snapshot.text.length, assets: (snapshot.assets || []).length, pdfs: pdfCount, discoveredLinks: links.length, requestedUrls: requests.length, externalLinksExcluded: unsafeLinks.length, manualLinksExcluded, maxAgeHours, ageHours: Number(ageHours.toFixed(2)), errors, warnings })
  } else results.push({ sourceId: source.id, errors, warnings })
}
