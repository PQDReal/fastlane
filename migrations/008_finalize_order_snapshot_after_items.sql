...
-- The order-item immutability trigger rejects INSERT/DELETE once the parent
-- order has snapshot_finalized_at set. Migration 005 inherited the earlier
-- checkout ordering and finalized the order before inserting its items, so
-- every real checkout failed with ORDER_ITEMS_FINALIZED. Keep the same
-- selected-items and option-snapshot contract, but finalize only after all
-- order-item snapshots have been inserted successfully.
...
-- Rollback guidance: reapplying migration 005 restores the prior function
-- definition, but that ordering is incompatible with the live
-- order_items_guard_snapshot trigger and makes checkout fail. Prefer rolling
-- the application back while retaining this database fix.
