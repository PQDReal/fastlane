-- Migration 047: forward-repair the contract issue RPC on databases where
-- migration 044 was applied before the inactive-document replay guard landed.
begin;
...
begin
  if to_regclass('public.deposit_orders') is null
    ...
    raise exception using
      errcode = 'P0001',
      message = 'MIGRATION_047_PREREQUISITE_TABLE_MISSING';
  end if;
end;
$preflight$;
...
comment on function public.issue_deposit_order_contract(uuid,text,uuid,text,text,text,jsonb,text,integer,text) is
  'Issues an immutable vehicle purchase document and rejects replay through a missing or inactive document.';
