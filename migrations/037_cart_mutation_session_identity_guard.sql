...
-- Keep the v1 signature stable. The application now reads users.id from the
-- encrypted Auth0 session, while this live guard keeps authorization in the
-- database and prevents a stale session identity from mutating a cart.
...
comment on function public.mutate_accessory_cart_item_v1(uuid, uuid, text, integer) is
  'Atomic accessory cart item mutation with live local-user authorization and canonical cart snapshot.';
